...
functions = {}
lineNum = 0 
IMPORT_LIST = []  # Empty list for imports - can be populated later
...
def install_command(import_name):
    """Install a command from the import list to commands folder"""
    if import_name not in IMPORT_LIST:
        print(f"Error: '{import_name}' not found in import list")
        return False
    
    print(f"Installing {import_name} to commands folder...")
    
    command_file = commands_dir / f"{import_name}.py"
    
    if not command_file.exists():
        template = f'''def run(tokens, variables, functions, get_val, errorLine, lineNum, line):
    """
    Command: {import_name}
    """
    print(f"Executing {import_name} command with args: {{tokens[1:]}}")
    return True
'''
        ...
        print(f"Command {import_name} already exists")
        return False
    ...
    while i < len(lines):
        ...
        try:
            ...
            if command == "function" and len(tokens) >= 4 and tokens[2] == "(" and ")" in tokens and "{" in line:
                ...
                if len(tokens) == 2:
                    import_name = tokens[1]
                    if install_command(import_name):
                        print(f"Command {import_name} installed successfully")
                    else:
