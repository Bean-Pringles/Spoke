...
def parse_if_else_chain(lines, start_idx, start_line_offset=0):
    ...
    while current_idx < len(lines):
        ...
        actual_line_num = start_line_offset + current_idx + 1
        ...
        if tokens[0] == "if" and "then" in tokens and "{" in line:
            try:
                start = tokens.index("(")
                end = tokens.index(")")
                inner_tokens = tokens[start + 1:end]
                cond = parse_condition(inner_tokens, lineNum, line)
            except Exception as e:
                print(f"DEBUG: Error parsing if condition on line {lineNum}: {e}")
                errorLine(lineNum, line)
            
            # Collect the if block
            if_block_lines, block_end = collect_block(lines, current_idx)
            
            if cond and not executed_any:
                execute_lines(if_block_lines, start_line_offset + current_idx)
                executed_any = True
        ...
        # Check if this is an else-if or else block
            ...
            else_line = line[1:].strip()  # Remove the }
            else_tokens = re.findall(r'"[^"]*"|\'[^\']*\'|-?\d+\.?\d*|<<|>>|<=|>=|==|!=|=<|=>|\w+|[=+*/()%<>{}:!@#$%^&-]', else_line)
            
            else_cond = False  # Default to false
            
            if len(else_tokens) > 1 and else_tokens[0] == "else" and else_tokens[1] == "if":
                # This is an else if - find the condition
                try:
                    ...
                    if len(condition_tokens) < 3:
                        print(f"DEBUG: Invalid else-if condition on line {lineNum}: {condition_tokens}")
                        errorLine(lineNum, line)
                    
                    else_cond = parse_condition(condition_tokens, lineNum, line)
                except Exception as e:
                    print(f"DEBUG: Error parsing else-if condition on line {lineNum}: {e}")
                    errorLine(lineNum, line)
            elif len(else_tokens) >= 1 and else_tokens[0] == "else" and "{" in line:
                # Plain else - condition is always true if we haven't executed anything
                else_cond = True
            else:
                print(f"DEBUG: Malformed else statement on line {lineNum}: {else_tokens}")
                errorLine(lineNum, line)
            
            # Collect the else/else-if block
            else_block_lines, block_end = collect_block(lines, current_idx)
            
            # Execute if condition is met and we haven't executed any block yet
            if else_cond and not executed_any:
                execute_lines(else_block_lines, start_line_offset + current_idx)
                executed_any = True
            
            current_idx = block_end
            
        # Check for standalone closing brace (end of if-else chain)
        elif line == "}":
            current_idx += 1
            break
            
        else:
            # Not part of this if-else chain
            break
