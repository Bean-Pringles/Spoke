...
def run(args):
    ...
    if action == "run":
        ...
        parent_dir = os.path.dirname(script_dir)
        commands_dir = os.path.join(parent_dir, "apps")
        app_path = os.path.join(commands_dir, f"{app_name}.py")

        if not os.path.isfile(app_path):
            print(f"App '{app_name}' not found in {commands_dir}")
            return

        try:
            # Launch the app script using the current Python interpreter
